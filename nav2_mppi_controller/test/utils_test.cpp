...
TEST(UtilsTests, WithTolTests)
{
  ...
  EXPECT_FALSE(withinPositionGoalTolerance(goal_checker, pose, goal_pose));
  EXPECT_FALSE(withinPositionGoalTolerance(0.25, pose, goal_pose));

  // Test in tolerance
  goal_pose.position.x = 9.8;
  goal_pose.position.y = 0.95;
  EXPECT_TRUE(withinPositionGoalTolerance(goal_checker, pose, goal_pose));
  EXPECT_TRUE(withinPositionGoalTolerance(0.25, pose, goal_pose));

  goal_pose.position.x = 10.0;
  goal_pose.position.y = 0.76;
  EXPECT_TRUE(withinPositionGoalTolerance(goal_checker, pose, goal_pose));
  EXPECT_TRUE(withinPositionGoalTolerance(0.25, pose, goal_pose));

  goal_pose.position.x = 9.76;
...
TEST(UtilsTests, FurthestAndClosestReachedPoint)
{
  ...
  geometry_msgs::msg::Pose dummy_goal2;
  CriticData data2 =
  {state, generated_trajectories, path, dummy_goal2, costs, model_dt, false, nullptr, nullptr,
    std::nullopt, std::nullopt};  /// Caution, keep references
  ...
  geometry_msgs::msg::Pose dummy_goal3;
  CriticData data3 =
  {state, generated_trajectories, path, dummy_goal3, costs, model_dt, false, nullptr, nullptr,
    std::nullopt, std::nullopt};  /// Caution, keep references
